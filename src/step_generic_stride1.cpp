...
/* These macros get into the guts of the S1LOOP_OVER_VOL loops to
...
*/
void step_curl_stride1(RPR f, component c, const RPR g1, const RPR g2, ptrdiff_t s1,
    ...
    if (dsigu == NO_DIRECTION) { // no fu update
      if (cnd) {
        ...
        if (g2) {
          ...
          S1LOOP_OVER_VOL_OWNED0(gv, c, i) { f[i] -= dtdx * (g1[i + s1] - g1[i]); }
        }
      }
    }
...
void step_beta_stride1(RPR f, component c, const RPR g, const grid_volume &gv, double betadt,
  ...
  if (dsig != NO_DIRECTION) { // PML in f update
    ...
    if (dsigu != NO_DIRECTION) { // fu, no PML in f
      ...
      if (cndinv) { // conductivity, no PML
        S1LOOP_OVER_VOL_OWNED0(gv, c, i) { f[i] += betadt * g[i] * cndinv[i]; }
      }
      else { // no conductivity or PML
        S1LOOP_OVER_VOL_OWNED0(gv, c, i) { f[i] += betadt * g[i]; }
      }
    }
  }
}
...
inline double calc_nonlinear_u(const double Dsqr, const double Di, const double chi1inv,
...
void step_update_EDHB_stride1(RPR f, component fc, const grid_volume &gv, const RPR g, const RPR g1,
...
#define OFFDIAG(u, g, sx)                                                                          \
  ...
  /* As with step_curl_stride1, these loops are all essentially copies
  ...
  if (dsigw != NO_DIRECTION) { //////// PML case (with fw) /////////////
    ...
    if (u1 && u2) { // 3x3 off-diagonal u
      ...
      if (chi3) {
        ...
        S1LOOP_OVER_VOL_OWNED(gv, fc, i) { f[i] = g[i]; }
    }
  }
}
